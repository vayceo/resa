...
void Stream::Push(const VoicePacket& packet)
{
    ...
    if(channelPtr == nullptr)
    {
        for(const auto& channel : this->channels)
        {
            ...
            {
                DLOG("[sv:dbg:stream:push] : channel %p was occupied by player %hu "
        ...
        for(const auto& channel : this->channels)
        {
            if(channelCount == this->channels.size())
            {
               ...
               // channel->SetStopCallback(std::bind(&Stream::OnChannelStop, this, std::placeholders::_1));
                ...
                DLOG("[sv:dbg:stream:push] : channel %p for player %hu created (stream:%s)",
...
{
    BASS_ChannelSetAttribute(channel.GetHandle(), BASS_ATTRIB_VOL, 1.f);
