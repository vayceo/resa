...
void CRGBA::RainbowCycle(uint32 t) {
    ...
    // Вычисляем прошедшее время с начала периода
    uint32_t timeElapsed = t % period;

    // Вычисляем прогресс в периоде от 0 до 1
    double progress = static_cast<double>(timeElapsed) / static_cast<double>(period);

    // Используем sin для плавного перехода между цветами
    double angle = progress * 2 * M_PI; // Преобразуем прогресс в угол (0 - 2?)
    r = sin(angle + 0) * 127 + 128; // Генерируем компоненты цвета
