...
class CLinkAddress {
public:
    CLinkAddress(size_t area, size_t nodeId) :
...
VALIDATE_SIZE(CPathNode, (VER_x32 ? 0x1C : 0x28));

class CPathFind {
    static constexpr auto NUM_TOTAL_PATH_NODE_AREAS = NUM_PATH_MAP_AREAS + NUM_PATH_INTERIOR_AREAS;
public:
    uint8           ColourGroups[2];
    CPathNode       *m_pathFindHashTable[512];
    CPathNode       *m_pPathNodes[NUM_TOTAL_PATH_NODE_AREAS];
    CCarPathLink    *pLinks[NUM_TOTAL_PATH_NODE_AREAS];
    CNodeAddress    *pAdjacentNodes[NUM_TOTAL_PATH_NODE_AREAS];
    uint8           *pAdjacentNodesDist[NUM_TOTAL_PATH_NODE_AREAS];
    uint8           *pAdjacentNodesFlags[NUM_TOTAL_PATH_NODE_AREAS];
    CLinkAddress    *pAdjacentLinks[NUM_TOTAL_PATH_NODE_AREAS];
    CPathNode       *pTNodes[NUM_TOTAL_PATH_NODE_AREAS];
    uint32          m_anNumNodes[NUM_TOTAL_PATH_NODE_AREAS]; // NumNodes
    uint32          m_anNumVehicleNodes[NUM_TOTAL_PATH_NODE_AREAS]; // NumNodesCarNodes
    uint32          m_anNumPedNodes[NUM_TOTAL_PATH_NODE_AREAS]; // NumNodesPedNodes;
    int32           NumCarPathLinks[NUM_TOTAL_PATH_NODE_AREAS];
    int32           NumAdjNodesOnList[NUM_TOTAL_PATH_NODE_AREAS];
    int32           OldAdjNodeIndex[64][16];
    int32           NewAdjNodeIndex[64][16];
    int32           m_totalNumNodesInPathFindHashTable; // SA NodesOnListDuringPathfinding;
    uint32          InteriorID[8];
    int32           NumNodeSwitches;
    CForbiddenArea  NodeSwitches[64];
    bool            bActiveRequestForRegions;
    float           RequestMinX;
    float           RequestMaxX;
    float           RequestMinY;
    float           RequestMaxY;
...
public:
    ...
    */
    ...
    CLinkAddress FindLinkBetweenNodes(CNodeAddress node1, CNodeAddress node2);
