...
CShadowCamera_VC::CShadowCamera_VC()
{
	m_pCamera  = nil;
	m_pTexture = nil;
}
...
RwCamera *
CShadowCamera_VC::Create(int32 rasterSize)
{
...
RwCamera *
CShadowCamera_VC::SetFrustum(float objectRadius)
{
	ASSERT(m_pCamera != nil);
...
RwCamera *
CShadowCamera_VC::SetLight(RpLight *light)
{
	ASSERT(light != nil);
	ASSERT(m_pCamera != nil);
...
RwCamera *
CShadowCamera_VC::SetCenter(RwV3d *center)
{
	ASSERT(center != nil);
	ASSERT(m_pCamera != nil);
...
RwCamera *
CShadowCamera_VC::Update(RpClump *clump)
{
	ASSERT(clump != nil);
	ASSERT(m_pCamera != nil);
...
RwCamera *
CShadowCamera_VC::Update(RpAtomic *atomic)
{
	ASSERT(atomic != nil);
	ASSERT(m_pCamera != nil);
...
RwRaster *
CShadowCamera_VC::MakeGradientRaster()
{
	ASSERT(m_pCamera != nil);
...
RwRaster *
CShadowCamera_VC::RasterResample(RwRaster *dstRaster)
{
...
RwRaster *
CShadowCamera_VC::RasterBlur(RwRaster *dstRaster, int32 numPasses)
{
	ASSERT(dstRaster != nil);
	ASSERT(m_pCamera != nil);
...
RwRaster *
CShadowCamera_VC::RasterGradient(RwRaster *dstRaster)
{
	ASSERT(dstRaster != nil);
	ASSERT(m_pCamera != nil);
...
RwRaster *CShadowCamera_VC::DrawOutlineBorder(RwRGBA const& color)
{
	ASSERT(m_pCamera != nil);
